...
my_local_download_dir_warc = "../data/cc_download_warc/"
# download dir for articles
my_local_download_dir_article = "../data/cc_download_articles/"

my_filter_start_date = datetime.datetime(2016, 9, 1)  # datetime.datetime(2016, 1, 1)
# end date (if None, any date is OK as end date), as datetime
my_filter_end_date = datetime.datetime(2019, 2, 28)  # datetime.datetime(2016, 12, 31)

my_json_export_style = 1  # 0 (minimize), 1 (pretty)


class CCCrawler(CommonCrawlExtractor):
    def filter_record(self, warc_record, article=None):
        filter_pass, article = super().filter_record(warc_record, article=article)
        if filter_pass and article is not None:
            filter_pass = article.language == "en"
        return filter_pass, article
...
def __get_remote_index(warc_files_start_date, warc_files_end_date):
    ...
    with tempfile.NamedTemporaryFile() as temp:
        ...
        if warc_files_start_date or warc_files_end_date:
            ...
            warc_dates = __iterate_by_month(
                start_date=warc_files_start_date, end_date=warc_files_end_date
            )
            for date in warc_dates:
                year = date.strftime("%Y")
                month = date.strftime("%m")
                cmd += (
                    "aws s3 ls --recursive s3://commoncrawl/crawl-data/CC-NEWS/%s/%s/ --no-sign-request >> %s && "
                    % (year, month, temp_filename)
                )
...
if __name__ == "__main__":
    crawl_from_commoncrawl(
        ...
        warc_files_start_date=None,
        warc_files_end_date=my_filter_end_date,
        strict_date=True,
        reuse_previously_downloaded_files=True,
        local_download_dir_warc=my_local_download_dir_warc,
        continue_after_error=True,
        show_download_progress=True,
        number_of_extraction_processes=88,
        log_level=logging.INFO,
        delete_warc_after_extraction=True,
        continue_process=True,
        fetch_images=False,
        extractor_cls=CCCrawler,
    )
