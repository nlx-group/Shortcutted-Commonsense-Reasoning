...
class BaseModelWrapper(ModelWrapper):
    def __init__(self, model_name, batch_size, num_choices, max_seq_len, ckpt_path):
...
class PIQAModelWrapper(BaseModelWrapper):
    def get_model(self):
        return RobertaForMultipleChoicePIQA.load_from_checkpoint(
            self.ckpt_path,
            hparams=Namespace(
...
class ARCTModelWrapper(BaseModelWrapper):
    def get_model(self):
        return RobertaForARCT.load_from_checkpoint(
            self.ckpt_path,
            hparams=Namespace(
                model_name=self.model_name,
                batch_size=self.batch_size,
...
class ARCModelWrapper(BaseModelWrapper):
    def get_model(self):
        return RobertaForRankingARC.load_from_checkpoint(
            self.ckpt_path,
            hparams=Namespace(
                model_name=self.model_name,
                batch_size=self.batch_size,
...
class CSQAModelWrapper(BaseModelWrapper):
    def get_model(self):
        return RobertaForCSQA.load_from_checkpoint(
            self.ckpt_path,
            hparams=Namespace(
                model_name=self.model_name,
                batch_size=self.batch_size,
...
def get_model(name, model_name, batch_size, num_choices, max_seq_len, ckpt_path):
    models = {
    ...
    return models[name](model_name, batch_size, num_choices, max_seq_len, ckpt_path)
...
def attack(
    ...
    model = get_model(
        task, model_name, batch_size, num_choices, max_seq_len, checkpoint_path
    )
